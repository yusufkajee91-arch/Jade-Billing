...
const log = apiLogger('trust-entries/[id]')
...
export async function GET(
  ...
  try {
    ...
    log.debug('GET trust entry id:', id)
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params
    log.debug('DELETE trust entry id:', id)
    ...
    if (entry.linkedEntryId) {
      log.warn('DELETE rejected: linked transfer entry, id =', id)
      return NextResponse.json(
