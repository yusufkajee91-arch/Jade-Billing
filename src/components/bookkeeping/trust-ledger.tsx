...
export function TrustLedger({ matterId, session }: Props) {
  ...
  const fetchEntries = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/trust-entries?matterId=${matterId}`)
      if (!res.ok) throw new Error('Failed to load trust entries')
      setEntries(await res.json())
    } catch {
      toast.error('Could not load trust ledger')
    } finally {
      setLoading(false)
    }
  }, [matterId])
  ...
  async function deleteEntry(id: string) {
    if (!confirm('Delete this trust entry? This cannot be undone.')) return
    const res = await fetch(`/api/trust-entries/${id}`, { method: 'DELETE' })
    if (!res.ok) {
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: '#8897C0' }} />
        ...
        <div className="flex items-center gap-3">
          <span
            ...
            style={{ color: totalBalance >= 0 ? '#8897C0' : '#9A3A3A' }}
          >
          ...
          {(session.user.role === 'admin' || session.user.role === 'fee_earner') && (
            <Button
              ...
              className="font-sans text-xs tracking-wide uppercase border-[#8897C0] text-[#8897C0] hover:bg-[hsl(225_40%_96%)]"
            >
      ...
      {loading ? (
        ...
        <div className="rounded-md border border-[hsl(225_35%_90%)] overflow-hidden">
          <table className="w-full text-xs">
            <thead>
              <tr style={{ backgroundColor: 'hsl(225 40% 96%)' }}>
            ...
            <tbody>
              {rows.map(({ entry, isInflow, balance }) => (
                <tr
                  key={entry.id}
                  className="border-t border-[hsl(225_35%_90%)] hover:bg-[hsl(225_40%_97%)] transition-colors"
                >
                  ...
                  <td
                    ...
                    style={{ color: balance >= 0 ? '#8897C0' : '#9A3A3A' }}
                  >
...
function TrustEntryForm({ matterId, onSaved, onCancel }: FormProps) {
  const [entryType, setEntryType] = useState('trust_receipt')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const res = await fetch('/api/trust-entries', {
        method: 'POST',
      ...
      toast.success('Trust entry recorded')
      onSaved()
    } finally {
      setSaving(false)
    }
  ...
  return (
    ...
    >
      <p className="font-sans text-[10px] tracking-widest uppercase text-[#8897C0] mb-1">
        New Trust Entry
      </p>
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          >
            ...
            <option value="trust_payment">Trust Payment</option>
            <option value="trust_transfer_in">Transfer In</option>
            <option value="trust_transfer_out">Transfer Out</option>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-1">
          <label className="font-sans text-xs text-muted-foreground block mb-1">Cheque No.</label>
          <input
            type="text"
            value={chequeNumber}
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Deposit received from client"
          required
          className={inputCls}
        />
      </div>
      <div className="flex gap-2 justify-end pt-1">
        ...
        <Button
          ...
          disabled={saving}
          className="font-sans text-xs tracking-wide uppercase"
          style={{ backgroundColor: '#8897C0', color: '#fff' }}
        >
